...
class Trainer(object):
    ...
    def _build_optimizer(self):
        ...
        if (not hasattr(self.args, "lr_list")) or (self.args.lr_list is None):
            ...
            lr_list = [float(lr.strip()) for lr in self.args.lr_list.split(",")]
        ...
        for params, curr_lr in zip(param_groups, lr_list):
            if self.args.fp16:
                ...
                if self.args.memory_efficient_fp16:
                    optimizer = optim.MemoryEfficientFP16Optimizer.build_optimizer(self.args, params)
                else:
                    optimizer = optim.FP16Optimizer.build_optimizer(self.args, params)
            else:
                if self.cuda and torch.cuda.get_device_capability(0)[0] >= 7:
    ...
    def set_current_optimizer(self):
        self._current_optimizer = self.task.choose_current_optimizer(self.optimizers)
    ...
    def save_checkpoint(self, filename, extra_state):
        ...
        if distributed_utils.is_master(self.args):  # only save one checkpoint
            ...
            checkpoint_utils.save_state(
                ...
                self.optimizers[0], self.lr_schedulers[0], self.get_num_updates(),
    ...
    def load_checkpoint(
        ...
        if last_optim_state is not None and not reset_optimizer:
            ...
            assert last_optim['optimizer_name'] == self.optimizers[0].__class__.__name__, \
                'Optimizer does not match; please reset the optimizer (--reset-optimizer).'

            if not reset_lr_scheduler:
                self.lr_schedulers[0].load_state_dict(last_optim['lr_scheduler_state'])
            self.optimizers[0].load_state_dict(last_optim_state, optimizer_overrides)
    ...
    def train_step(self, samples, dummy_batch=False, raise_oom=False):
            ...
            try:
                with maybe_no_sync():
                    ...
                    loss, sample_size, logging_output = self.task.train_step(
                        sample, self.model, self.criterion, self.current_optimizer,
                        ignore_grad
                    )
        ...
        try:
            ...
            if sample_size > 0:
                self.current_optimizer.multiply_grads(self.args.distributed_world_size / float(sample_size))

            # clip grads
            grad_norm = self.current_optimizer.clip_grad_norm(self.args.clip_norm)
        ...
        if self.args.fp16:
            ...
            self.meters['loss_scale'].update(self.current_optimizer.scaler.loss_scale)

        self.clear_buffered_stats()
        self.meters['train_wall'].stop()

        # set the new current optimizer if there is a change
        self.set_current_optimizer()
    ...
    def lr_step_update(self):
        ...
        lr = 0.0
        for lrs in self.lr_schedulers:
            lr = lrs.step_update(self.get_num_updates())
        return lr

    def get_lr(self):
        """Get the current learning rate."""
        return self.current_optimizer.get_lr()
